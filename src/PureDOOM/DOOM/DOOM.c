...
int doom_read_impl(void* handle, void *buf, int count)
{
    return nk_fs_read(handle, buf, count);
}
int doom_write_impl(void* handle, const void *buf, int count)
{
    return nk_fs_write(handle, buf, count);
}
int doom_seek_impl(void* handle, int offset, doom_seek_t origin)
{
    return nk_fs_seek(handle, offset, origin);
}
int doom_tell_impl(void* handle)
{
    return nk_fs_tell(handle);
}
int doom_eof_impl(void* handle)
{
    ERROR_PRINT("UNIMPLEMENTED FUNCTION: eof\n");
    return 1;
}
#endif
...
#define NS_PER_SEC 1000000000UL
    uint64_t ns = ts.tv_sec * NS_PER_SEC + ts.tv_nsec;
    ns /= 1; // optionally dilate time
    *sec = ns /  NS_PER_SEC;
    *usec = (ns % NS_PER_SEC) / 1000;
    /*DEBUG_PRINT("time: (%ld, %ld)\n", *sec, *usec);*/
    /**sec = 0;*/
    /**usec = 0;*/
}
#endif
